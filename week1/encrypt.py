...
def populatePT(position, values, i, PT, CT,key):
    PT[position][i]=" "
    for m in range(0, (len(values)), 2):
        temprow=values[1+m]
        value=values[m]
        PT[temprow][i]=value
    key[position][i]=strxor((PT[position][i]), CT[position][i])
    return PT,key
    

def spacexorxor(lenCT,CTarray, ogCT):
    space=ord(" ")
    PT=[["*" for k in range(lenCT)] for l in range(len(CTarray))]
    key=[["*" for k in range(lenCT)] for l in range(len(CTarray))]
            ...
            for k in range(0, len(CTarray)):
                if j==k: # as we are about to xor CT's with each other
                    ...
                    if ((65 <= result <= 90) or (97<= result <=122)):
                        templist[j].extend((chr(result),k))
        #print (np.array(templist))
        position=findspace(templist)
        PT,key=populatePT(position, templist[position],i,PT,ogCT,key)
    print(''.join(PT[0]))
    print(''.join(PT[1]))
    print(''.join(PT[2]))
    print(''.join(PT[3]))
...
def convertCTtobytes(CTarray):
    ...
    PTsofar=spacexorxor(int(shortestCT), CTbytes, CTarray)
